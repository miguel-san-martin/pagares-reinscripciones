...
export class CampamentoIestService extends ServicioBase {
  ...
  public CheckIfIsEditable(
    idPeriodo: string = '', // envia un character numero o nombre
  ...
  public GetBaseFee(
    idPeriodo: string, // enviá un character numero o nombre
  ...
  public updateCost(
    idCosto: string,
    costo: string, // envia un carcacter numero o nombre
  ): Observable<any[]> {
    const parametros = {
      servicio: 'costos',
  ...
  public updateDescount(
    ...
    return this.consulta(
      { ...parametros, idDescuento, costo },
      '/api/CAMP/CAMP.php',
    );
  }

  public closePeriod(idRegistroAdmin: string = ''): Observable<any[]> {
    const parametros = {
      servicio: 'costos',
